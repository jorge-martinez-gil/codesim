# -*- coding: utf-8 -*-
"""
Bag-of-Words Similarity Detection for Java Code (Alternative implementation)
...
def extract_semantic_tokens(code_snippet):
    tokens = list(javalang.tokenizer.tokenize(code_snippet))
    semantic_tokens = []

    for token in tokens:
        if isinstance(token, javalang.tokenizer.Identifier):
            semantic_tokens.append(f"Identifier:{token.value}")
        elif isinstance(token, javalang.tokenizer.Keyword):
            semantic_tokens.append(f"Keyword:{token.value}")
        elif isinstance(token, javalang.tokenizer.Operator):
            semantic_tokens.append(f"Operator:{token.value}")
        elif isinstance(token, javalang.tokenizer.Literal):
            semantic_tokens.append(f"Literal:{token.value}")
        else:
            semantic_tokens.append(f"Other:{token.value}")
                        ...
                        for root, dirs, files in os.walk(subfolder_path):
                            for java_file in files:
                                if java_file.endswith('.java'):
                                    ...
                                    semantic_tokens1 = extract_semantic_tokens(code1)
                                    semantic_tokens2 = extract_semantic_tokens(code2)
                                    vectorizer = CountVectorizer()
                                    dtm = vectorizer.fit_transform([semantic_tokens1, semantic_tokens2])
                                    similarity_matrix = cosine_similarity(dtm)
                                    similarity_ratio = similarity_matrix[0, 1]
